...
int name_release(struct conn *conn, const char *name)
{
	...
	cmd_name = (struct kdbus_cmd_name *)alloca(size);
		    ...
		    uint64_t dst_id)
{
	...
	msg = (struct kdbus_msg *)malloc(size);
	if (!msg) {
...
struct kdbus_item *make_policy_name(const char *name)
{
	...
	p = (struct kdbus_item*)malloc(size);
	if (!p)
		return NULL;
	memset(p, 0, size);
	p->size = size;
...
int upload_policy(int fd, const char *name)
{
	...
	policy = make_policy_access(KDBUS_POLICY_ACCESS_USER, KDBUS_POLICY_RECV | KDBUS_POLICY_OWN | KDBUS_POLICY_SEND, getuid());
	append_policy(cmd_policy, policy, size);

/*
	policy = make_policy_access(KDBUS_POLICY_ACCESS_USER, KDBUS_POLICY_SEND, getuid());
	append_policy(cmd_policy, policy, size);
