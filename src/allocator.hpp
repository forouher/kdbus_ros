...
#ifndef BOOST_INTERPROCESS_ALLO1CATOR_HPP
#define BOOST_INTERPROCESS_ALLO1CATOR_HPP
...
//   typedef boost::interprocess::version_type<ros_allocator, 2>   version;
   ...
   pointer allocate(size_type count, cvoid_ptr hint = 0)
   {
      ...
      if (mp_mngr)
        ...
        return boost::interprocess::offset_ptr<T>(std::allocator<T>().allocate(count, hint.get()));
      }
   }
   ...
   { if (mp_mngr)
	 mp_mngr->deallocate((void*)ipcdetail::to_raw_pointer(ptr));
        ...
        std::allocator<T>().deallocate(ptr.get(),size);
}
