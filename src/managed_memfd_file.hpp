...
template
      ...
      >
class basic_managed_memfd_file
      ...
      ,ipcdetail::mfile_open_or_create<AllocationAlgorithm>::type::ManagedOpenOrCreateUserOffset>
{
   ...
   basic_managed_memfd_file *get_this_pointer()
   {  return this;   }

   private:
   typedef typename base_t::char_ptr_holder_t   char_ptr_holder_t;
   BOOST_MOVABLE_BUT_NOT_COPYABLE(basic_managed_memfd_file)
   /// @endcond
   ...
   //!This can throw.
   basic_managed_memfd_file()
   {}

   //!Creates mapped file and creates and places the segment manager.
   //!This can throw.
   basic_managed_memfd_file(create_only_t, const char *name,
   ...
   //!segment.
   //!This can throw.
   basic_managed_memfd_file (open_or_create_t,
   ...
   //!This can throw.
   basic_managed_memfd_file (open_only_t, const char* name,
                              const void *addr = 0)
   ...
   //!This can throw.
   basic_managed_memfd_file (open_copy_on_write_t, const char* name,
                              const void *addr = 0)
   ...
   //!in read-only mode.
   //!This can throw.
   basic_managed_memfd_file (open_read_only_t, const char* name,
                              const void *addr = 0)
   ...
   //!Does not throw
   basic_managed_memfd_file(BOOST_RV_REF(basic_managed_memfd_file) moved)
   {
      this->swap(moved);
   }
   ...
   //!Moves the ownership of "moved"'s managed memory to *this.
   //!Does not throw
   basic_managed_memfd_file &operator=(BOOST_RV_REF(basic_managed_memfd_file) moved)
   {
      basic_managed_memfd_file tmp(boost::move(moved));
      this->swap(tmp);
      return *this;
   }
   ...
   //!Never throws.
   void swap(basic_managed_memfd_file &other)
   {
      base_t::swap(other);
   ...
   static bool grow(const char *filename, size_type extra_bytes)
   {
      return base_t::template grow
         <basic_managed_memfd_file>(filename, extra_bytes);
   }
   ...
   static bool shrink_to_fit(const char *filename)
   {
      return base_t::template shrink_to_fit
         <basic_managed_memfd_file>(filename);
   }
...
typedef basic_managed_memfd_file
   <char
